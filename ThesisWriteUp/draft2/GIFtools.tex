...
\hline
\end{tabular}
\caption{An example survey file from TKC bore holes (Depth is the position along the hole where the change indirrection occurs (in meters), Azimuth and Dip are the new direction at the depth provided (degrees). The holes are the same as \autoref{tab:collarEx}}
\label{tab:surveyEx}
\end{fileExample}
...
\hline
\end{tabular}
\caption{An example survey file from TKC bore holes (From and To are depth along the hole in meters and Litho is in this case a lithology unit. Other properties can be included is whatever unit is appropriate. The holes are the same as \autoref{tab:collarEx})}
\label{tab:propEx}
\end{fileExample}
...
\begin{figure} [h]
    ...
    \caption{Example of a geological definition as displayed in the GIFtools GUI, the Mean Property, and Bounds are susceptibility measurement in SI$\times10^-3$ and the WS is to set the smallness weight model later on.}
...
\begin{equation}
 \phi_{k\text{-}mean} = \sum_{j=1}^k\sum_{m_i \in S_j}\|m_i - v_j\|_2^2,
\end{equation}
\label{eq:kmean}
